...
export const Radio: React.FC<RadioProps> = (props) => {
  ...
  return (
    <div
      data-testid="Wps-Radio"
      className={classnames("Wps-Radio", className)}
      {...(otherProps as any)}
    >
      <style jsx>{`
        .Wps-Radio {
        ...
        .Wps-Radio input:checked ~ .Wps-RadioIndicator {
          ${handleBackgroundColor()}
        }
        .Wps-Radio input:checked ~ .Wps-RadioIndicator:after {
          display: block;
        }
        .Wps-Radio input:disabled ~ .Wps-RadioIndicator {
          ${handleDisabledColor()}
          opacity: 0.6;
          pointer-events: none;
        }

        .Wps-Radio .Wps-RadioIndicator {
          display: flex;
          align-items: center;
          justify-content: center;
          position: relative;
        ...
        .Wps-Radio .Wps-RadioIndicator:after {
          content: "";
          position: absolute;
          display: none;
        }

        .Wps-Radio :hover input ~ .Wps-RadioIndicator,
        .Wps-Radio input:focus ~ .Wps-RadioIndicator {
          ${handleDefaultHoverColor()}
        }
        .Wps-Radio :hover input:not([disabled]):checked ~ .Wps-RadioIndicator,
        .Wps-Radio input:checked:focus ~ .Wps-RadioIndicator {
          ${handleHoverColor()}
        }
        ...
        .Wps-Radio input:disabled ~ .Wps-RadioIndicator:after {
          ${handleIndicatorColor()}
        }
        .Wps-Radio input:disabled ~ .Wps-RadioIndicator {
          ${handleIconColor()}
      ...
      <input
        ...
        {...inputProps}
      />
      <label className="Wps-RadioLabel">{label}</label>
      <div className="Wps-RadioIndicator">{checked && icon}</div>
    </div>
  );
};
