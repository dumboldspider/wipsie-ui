...
import { EditorBeforeOptions } from "../types/editor";
...
export function getPointBefore(
  ...
  const lengthOfPointAndTextStackForUnitOffset =
    (typeof match === "string" ? match.length : 0) + 1;
  const pointAndTextStackForUnitOffset =
    unit === "offset"
      ? Array<StackItemForUnitOffset | undefined>(
          lengthOfPointAndTextStackForUnitOffset
        )
      : undefined;
  ...
  while (true) {
    ...
    if (
      !beforePoint ||
      (!acrossPaths && !Path.equals(beforePoint.path, previousBeforePoint.path))
    ) {
      return;
    }
    ...
    if (matchFn({ beforeString, beforePoint, at })) {
      return afterMatch
        ? pointAndTextStackForUnitOffset
          ? pointAndTextStackForUnitOffset[
              pointAndTextStackForUnitOffset.length - 1
            ]?.point
          : previousBeforePoint
        : beforePoint;
    }
