export const IndentationPlugin = (...options: any) => ({
  onKeyDown(event, editor) {
    // const allowedNodeTypes = ["paragraph", "heading"];
    // const eventKey = "Tab";
    // let allowed = false;
    // allowedNodeTypes.forEach((nodeType) => {
    //   if (!isSelectionMatch(editor, nodeType)) {
    //     allowed = true;
    //   }
    // });
    // const indentation = "";
    // if (event.key === eventKey && !event.shiftKey && allowed) {
    //   event.preventDefault();
    //   Transforms.insertText(editor, indentation);
    // } else if (event.key === eventKey && allowed) {
    //   event.preventDefault();
    //   const currentText = Editor.alert(currentText);
    //   Transforms.insertText(editor, currentText );
    // }
  },
});
